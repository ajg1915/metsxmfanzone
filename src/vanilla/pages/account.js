...
const PLANS = [
  {
    ...
    price: "$3.99",
    period: "per week",
    features: ["All live streams", "Members news", "Community access", "Cancel anytime"],
  },
  {
    id: "premium",
    name: "Premium",
    price: "$9.99",
    period: "per month",
    features: ["All live streams and replays", "Members news and podcasts", "Community and gameday", "Cancel anytime"],
    featured: true,
  },
  {
    id: "annual",
    ...
    price: "$129.99",
    period: "per year",
    features: ["Everything in Premium", "Best value for the full season", "Priority support"],
  },
];
...
export const renderDashboard = async (root) => {
  ...
  const tier = subscription?.plan_type || state.profile?.subscription_tier || "none";
  ...
  const content = `
    ...
    <section class="content-width card-panel">
      ...
      ${
        state.isMember
          ? '<a class="button secondary" href="/help/cancel-subscription">Manage membership</a>'
          : '<a class="button primary" href="/plans">Choose a plan</a>'
      }
    </section>
    <section class="content-width card-panel">
      <h2>Quick links</h2>
