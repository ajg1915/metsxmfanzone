...
const LiveStreamsSection = () => {
  ...
  const fetchStreams = async () => {
    try {
      ...
      const excludedPages = ['mlb-network', 'sny-tv', 'sny.tv', 'msg-network', 'msg'];
      const filtered = sorted.filter(s => {
        const title = s.title.toLowerCase();
        const isSportsNetwork24x7 =
          (title.includes('mlb network') || title.includes('sny.tv') || title.includes('msg network')) && title.includes('24/7');
